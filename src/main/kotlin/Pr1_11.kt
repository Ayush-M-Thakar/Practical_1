class Matrix(private val matrix: Array<IntArray>, private val rows: Int, private val cols: Int) {
    ...
    operator fun plus(obj: Matrix): Matrix {
        ...
        for (i in 0 until this.rows) { for (j in 0 until this.cols) {
    ...
    operator fun times(obj: Matrix): Matrix {
        ...
        for (i in 0 until this.rows) { for (j in 0 until obj.cols) {
            mul[i][j] = 0
            for (k in 0..obj.cols) {
...
fun main() {
    val firstMatrix = Matrix(arrayOf(intArrayOf(3, -2, 5), intArrayOf(3, 0,
        4)), 2, 3)
